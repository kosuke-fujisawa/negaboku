...
var test_choices: Array = [
	"協力的な選択肢（関係値+25）", "普通の選択肢（変化なし）", "対立的な選択肢（関係値-25）", "条件が必要な選択肢（親密レベル必要）"
]
...
func setup_game_manager():
	...
	game_manager.relationship_system.relationship_level_changed.connect(
		_on_relationship_level_changed
	)
...
func update_relationship_display():
	...
	if OS.is_debug_build() and relationship_label:
		...
		var rel_level = game_manager.relationship_system.get_relationship_level_string(
			"player", "partner"
		)
...
func _on_test_choices_pressed():
	...
	var rel_level_str = game_manager.relationship_system.get_relationship_level_string(
		"player", "partner"
	)
	if rel_level_str == "親密":
...
func _on_test_battle_pressed():
	...
	# バトル開始
	game_manager.battle_system.initialize(
		game_manager.party_members, game_manager.relationship_system
	)
...
func _on_relationship_level_changed(
	char1_id: String, char2_id: String, old_level: String, new_level: String
):
...
func start_demo_cycle():
	...
	# 1. ダイアログ表示
	dialogue_box.show_dialogue(["デモンストレーションを開始します。", "これから選択肢が表示されます。"], "システム")

	await dialogue_box.dialogue_finished

	# 2. 選択肢表示
	choice_panel.show_choices(["友好的に接する", "普通に接する", "冷たく接する"])

	await choice_panel.choice_selected

	# 3. 結果ダイアログ
	dialogue_box.show_dialogue(["選択の結果が関係値に反映されました。", "デモンストレーション完了です。"], "システム")
