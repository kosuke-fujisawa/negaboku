...
# システム参照（GameManager無効化のため型なしに変更）
var game_manager
...
func _ready():
	print("Phase3TestScene: GameManager無効化のため初期化をスキップ")
	return
...
func setup_ui():
	...
	transition_type.add_item("フェード", 0)
	transition_type.add_item("左スライド", 1)
	transition_type.add_item("右スライド", 2)
	transition_type.add_item("上スライド", 3)
	transition_type.add_item("下スライド", 4)
	transition_type.add_item("ディゾルブ", 5)
	transition_type.add_item("即座", 6)
