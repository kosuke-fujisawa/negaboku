...
func _ready():
	...
	if log_button:
		print("DialogueBox: ログボタンの詳細 - 可視: %s, 有効: %s, 親: %s" % [str(log_button.visible), str(not log_button.disabled), str(log_button.get_parent().name if log_button.get_parent() else "なし")])
...
func setup_ui() -> void:
	...
	print("DialogueBox: ログボタンを作成しました - 位置: %s, サイズ: %s" % [str(log_button.position), str(log_button.size)])
