...
func check_choice_condition(
	choice_data: Dictionary, relationship_system, party_members: Array
) -> bool:
	# 関係値条件のチェック
	if choice_data.has("required_relationship_level"):
		...
		if char1_id != "" and char2_id != "" and required_level != "":
			var current_level = relationship_system.get_relationship_level_string(
				char1_id, char2_id
			)
