...
func _create_ui_nodes():
	...
	print(
		"SimpleTextScene: テキストウィンドウ作成完了: pos=%s, size=%s" % [text_window.position, text_window.size]
	)
...
func _advance_text():
	...
	if is_text_animating:
		...
		print(
			(
				"SimpleTextScene: テキスト進行条件が満たされていません (is_animating=%s, continue_visible=%s)"
				% [is_text_animating, continue_indicator.visible]
			)
		)
...
func _start_text_animation():
	...
	text_tween.tween_method(
		_update_text_display, 0, current_text.length(), current_text.length() * text_speed
	)
...
func _add_to_log(speaker_name: String, text: String):
	# テキストログに追加#
	var log_entry = {
		"speaker": speaker_name, "text": text, "timestamp": Time.get_datetime_string_from_system()
	}
