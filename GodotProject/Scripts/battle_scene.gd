...
# BattleSystemスクリプトの再利用
var battle_script = preload("res://Scripts/systems/battle_system.gd")
...
func update_skill_button_state(character: Character):
	...
	# 共闘技または対立技が使用可能かチェック
	var can_coop = relationship_system.can_use_cooperation_skill(
		character.character_id, partner.character_id
	)
	var can_conflict = relationship_system.can_use_conflict_skill(
		character.character_id, partner.character_id
	)
...
func _on_skill_pressed():
	...
	# 関係値に基づいてスキルタイプを決定
	var skill_id = ""
	if relationship_system.can_use_cooperation_skill(
		current_turn_character.character_id, partner.character_id
	):
		skill_id = "cooperation_skill"
	elif relationship_system.can_use_conflict_skill(
		current_turn_character.character_id, partner.character_id
	):
...
func update_status_display():
	...
	if relationship_system and party_members.size() >= 2:
		var rel_value = relationship_system.get_relationship(
			party_members[0].character_id, party_members[1].character_id
		)
		var rel_level = relationship_system.get_relationship_level_string(
			party_members[0].character_id, party_members[1].character_id
		)
