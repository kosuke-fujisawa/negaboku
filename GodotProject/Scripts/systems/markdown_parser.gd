...
func _validate_command(element: ParsedElement, validation_result: Dictionary):
	...
	match element.content:
		...
		"bgm":
			_validate_audio_command(element, validation_result, "BGM")
		"se":
			_validate_audio_command(element, validation_result, "効果音")
		"voice":
			_validate_audio_command(element, validation_result, "音声")
		_:
			validation_result.warnings.append("未知のコマンド: %s" % element.content)


func _validate_audio_command(
	element: ParsedElement, validation_result: Dictionary, command_type: String
):
	# 音響コマンドの検証
	var has_play = element.parameters.has("play")
	var has_stop = element.parameters.has("stop")

	# playまたはstopのいずれかが必要
	if not has_play and not has_stop:
		validation_result.errors.append("%sコマンドにはplayまたはstopパラメータが必要です" % command_type)
		return

	# playとstopの同時指定は不正
	if has_play and has_stop:
		validation_result.errors.append("%sコマンドのplayとstopパラメータは同時に指定できません" % command_type)
		return

	# playパラメータがある場合の検証
	if has_play:
		var play_value = element.parameters.get("play", "")
		if play_value.is_empty():
			validation_result.errors.append("%sコマンドのplayパラメータにファイル名が必要です" % command_type)

		# volumeパラメータの検証（オプション）
		if element.parameters.has("volume"):
			var volume_str = element.parameters.get("volume", "")
			if not volume_str.is_valid_float():
				validation_result.warnings.append("%sコマンドのvolumeパラメータは数値である必要があります" % command_type)
			else:
				var volume = volume_str.to_float()
				if volume < 0.0 or volume > 1.0:
					validation_result.warnings.append(
						"%sコマンドのvolumeパラメータは0.0-1.0の範囲である必要があります" % command_type
					)

		# loopパラメータの検証（BGMのみ）
		if element.content == "bgm" and element.parameters.has("loop"):
			var loop_str = element.parameters.get("loop", "")
			if loop_str != "true" and loop_str != "false":
				validation_result.warnings.append("BGMコマンドのloopパラメータはtrueまたはfalseである必要があります")

	# stopパラメータがある場合の検証
	if has_stop:
		# fadeパラメータの検証（オプション）
		if element.parameters.has("fade"):
			var fade_str = element.parameters.get("fade", "")
			if not fade_str.is_valid_float():
				validation_result.warnings.append("%sコマンドのfadeパラメータは数値である必要があります" % command_type)
