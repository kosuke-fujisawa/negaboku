...
# アセットタイプ
enum AssetType { BACKGROUND, CHARACTER, EFFECT, UI }
...
func get_character_texture(
	character_name: String, face_expression: String = "normal"
) -> Dictionary:
	# キャラクターテクスチャを取得
	# Returns: {"texture": Texture2D, "is_fallback": bool, "source_path": String}
...
func get_asset_texture(asset_type: AssetType, asset_name: String) -> Dictionary:
	# 汎用アセットテクスチャ取得#
	var result = {"texture": null, "is_fallback": false, "source_path": "", "error_message": ""}
...
func _generate_fallback_texture(
	asset_type: AssetType, asset_name: String, cache_key: String
) -> Dictionary:
	# フォールバックテクスチャを生成#
	var result = {
		"texture": null,
...
func test_asset_loading():
	...
	for bg_name in test_backgrounds:
		...
		print(
			(
				"背景 %s: %s (fallback: %s)"
				% [bg_name, "成功" if result.texture else "失敗", result.is_fallback]
			)
		)

	# キャラクターテスト
	var test_characters = [
		{"name": "souma", "face": "normal"}, {"name": "nonexistent", "face": "normal"}
	]
	for char_data in test_characters:
		var result = get_character_texture(char_data.name, char_data.face)
		print(
			(
				"キャラクター %s_%s: %s (fallback: %s)"
				% [
					char_data.name,
					char_data.face,
