...
signal relationship_level_changed(
	char1_id: String, char2_id: String, old_level: String, new_level: String
)

# 関係値レベルの定義
enum RelationshipLevel { HOSTILE = -1, COLD = 0, NORMAL = 1, FRIENDLY = 2, INTIMATE = 3 }  # 敵対（0〜-25）  # 冷淡（1〜25）  # 普通（26〜50）  # 友好（51〜75）  # 親密（76〜100）
...
func set_relationship(char1_id: String, char2_id: String, value: int) -> bool:
	if char1_id.is_empty() or char2_id.is_empty():
		push_error(
			"RelationshipSystem: キャラクターIDが空です - char1: '%s', char2: '%s'" % [char1_id, char2_id]
		)
		return false
	...
	if old_value != clamped_value:
		...
		if old_level != new_level:
			relationship_level_changed.emit(
				char1_id, char2_id, level_to_string(old_level), level_to_string(new_level)
			)
...
# 関係値の変更
func modify_relationship(
	char1_id: String, char2_id: String, delta: int, reason: String = ""
) -> bool:
	if char1_id.is_empty() or char2_id.is_empty():
		push_error(
			"RelationshipSystem: キャラクターIDが空です - char1: '%s', char2: '%s'" % [char1_id, char2_id]
