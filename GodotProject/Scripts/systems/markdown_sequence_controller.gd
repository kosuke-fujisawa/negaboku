...
# 実行状態
enum State { IDLE, LOADING, EXECUTING, PAUSED, COMPLETED }  # 待機中  # ファイル読み込み中  # 順次実行中  # 一時停止  # 実行完了
...
func _execute_next_element():
	...
	print(
		(
			"要素実行 [%d/%d]: %s"
			% [current_element_index + 1, current_elements.size(), _element_to_string(element)]
		)
	)
...
func get_progress() -> Dictionary:
	...
	return {
		...
		"progress_percent":
		(
			(float(current_element_index) / float(current_elements.size())) * 100.0
...
func debug_print_state():
	...
	print(
		(
			"Progress: %d/%d (%.1f%%)"
			% [current_element_index, current_elements.size(), get_progress().progress_percent]
		)
	)
