		...
		match command.content:
			...
			"chara_show":
				if command.parameters.has("name") and command.parameters.has("pos"):
					var character_path = _resolve_character_path(
						command.parameters["name"], command.parameters.get("face", "normal")
					)
...
func _resolve_character_path(character_name: String, face_expression: String = "normal") -> String:
	...
	# キャラクター画像のデフォルトパス
	var full_path = (
		"res://Assets/images/characters/%s_%s.png" % [character_name.to_lower(), face_expression]
	)
	...
	if ResourceLoader.exists(full_path):
		...
		# フォールバック: 通常表情
		var fallback_path = (
			"res://Assets/images/characters/%s_normal.png" % character_name.to_lower()
		)
...
func load_scenario_library(
	config_file_path: String = "res://Assets/scenarios/library_config.json"
) -> bool:
...
func load_scenario_sequence(scenario_ids: Array) -> Dictionary:
	...
	for scenario_id in scenario_ids:
		...
		sequence_data[scenario_id] = {
			"scenario_data": scenario_data, "scene_count": converted_scenes.size()
		}
...
func test_scenario_library():
	...
	print(
		"シーケンステスト: %d シナリオ, %d シーン" % [sequence["scenarios"].size(), sequence["all_scenes"].size()]
	)
