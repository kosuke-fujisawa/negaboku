...
var enabled_choices: Array[bool] = []
...
func show_choices(choices: Array[String], enabled_states: Array[bool] = [], extra_data: Array[Dictionary] = []):
	if choices.is_empty():
		return
...
# 選択肢の条件チェック機能
func update_choice_conditions(relationship_system: RelationshipSystem, party_members: Array[Character]):
	for i in current_choices.size():
...
func check_choice_condition(choice_data: Dictionary, relationship_system: RelationshipSystem, party_members: Array[Character]) -> bool:
	# 関係値条件のチェック
