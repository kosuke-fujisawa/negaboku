...
func setup_turn_queue():
	...
	all_combatants.append_array(party_members)
...
func start_next_turn():
	...
	if turn_queue.is_empty():
		setup_turn_queue()  # 全員のターンが終わったら再度セットアップ
...
func perform_attack(actor: Character, targets: Array[Character]):
	if targets.is_empty():
		return
...
func check_relationship_skill_condition(actor: Character, skill) -> bool:
...
func process_ai_turn(ai_character: Character):
	...
	# 簡単なAI：体力が低い敵を優先攻撃
	var targets = party_members.filter(func(c): return c.current_hp > 0)
	if targets.is_empty():
		return
...
func apply_skill_effects(actor: Character, skill, targets: Array[Character]):
	# スキル効果の適用（仮実装）
...
func process_skill_relationship_effects(actor: Character, skill):
	var partner = get_partner(actor)
