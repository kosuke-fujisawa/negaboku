...
func _on_test_choices_pressed():
	...
	# 選択肢の有効状態を関係値に基づいて決定
	var enabled_states: Array[bool] = [true, true, true, false]
	
	# 親密レベルの場合は4番目の選択肢を有効化
	var rel_level = game_manager.relationship_system.get_relationship_level_enum("player", "partner")
	if rel_level == RelationshipSystem.RelationshipLevel.INTIMATE:
